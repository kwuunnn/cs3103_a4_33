...
def plot_latency_over_time(data):
    ...
    if unreliable:
        ...
        plt.scatter(u_times, u_latencies, alpha=0.6, s=30, label='Unreliable', color='orange')
...
def plot_latency_distribution(data):
    ...
    if unreliable:
        ...
        axes[1].axvline(np.mean(unreliable), color='red', linestyle='--',
...
def plot_sequence_timeline(data):
    ...
    if unreliable:
        u_times = [(d['arrival'] - data[0]['arrival']) / 1000 for d in unreliable]
        u_seqs = [d['seq'] for d in unreliable]
        plt.scatter(u_times, u_seqs, alpha=0.6, s=30, label='Unreliable', color='orange')
    
    plt.xlabel('Time (seconds)', fontsize=12)
    plt.ylabel('Sequence Number', fontsize=12)
    plt.title('Packet Arrival Order', fontsize=14, fontweight='bold')
    plt.legend()
    plt.grid(True, alpha=0.3)
    plt.tight_layout()
    plt.savefig('sequence_timeline.png', dpi=150)
    print("✓ Saved: sequence_timeline.png")
...
def print_statistics(data):
    ...
    if all_seqs:
        ...
        if missing:
            ...
            print(f"   Packet loss: {len(missing)} packets ({len(missing)/len(expected)*100:.1f}%)")
